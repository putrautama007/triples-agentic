...
- `.claude/agents/` — 13 TripleS specialist subagents (PRD, design, RFC, tasks, implementation, checker, test case, QA), each invoked via the Agent tool with its own pinned model.
...
- Or invoke a specialist subagent directly via the Agent tool when you need a single stage:
  ...
  - `dahyun-checker` — code quality checks: tests, types, lint (sonnet)
